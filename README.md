...
![Release](https://img.shields.io/github/v/release/youzhiqingw/proxy-installer?label=release)
...
- **密码加密存储** — AES-GCM 加密 + 系统密钥环保护，SSH 密码不落明文
- **安全随机数生成** — Reality 密钥和 UUID 使用 `crypto/rand` 加密安全随机源
- **日志脱敏处理** — 自动过滤密码、令牌、私钥、UUID 等敏感信息
- **SHA256 完整性校验** — sing-box 二进制下载后自动验证哈希值
- **VPS 续费成本管理** — 成本中心支持多实例录入、规格与价格记录、按币种月/年支出汇总、到期提醒和厂商聚合
...
| TUIC | QUIC 低延迟，适合快速连接 | Shadowrocket、Clash Meta / Mihomo、V2rayNG、sing-box |
...
Proxy Installer 生成的订阅链接遵循各协议的标准 URI 格式：

| 协议 | URI 格式示例 |
| --- | --- |
| Hysteria2 | `hysteria2://<password>@<host>:<port>/?insecure=1&sni=<sni>#<name>` |
| VLESS Reality | `vless://<uuid>@<host>:<port>?encryption=none&flow=xtls-rprx-vision&security=reality&sni=<sni>&fp=chrome&pbk=<publickey>&sid=<shortid>&type=tcp#<name>` |
| Trojan | `trojan://<password>@<host>:<port>?security=tls&sni=<sni>&allowInsecure=1#<name>` |
| Shadowsocks | `ss://<base64(method:password@host:port)>#<name>` |
| VMess | `vmess://<base64(JSON 配置)>` |
| TUIC | `tuic://<uuid>:<password>@<host>:<port>?congestion_control=bbr&udp_relay_mode=native&sni=<sni>&allow_insecure=1#<name>` |

> VMess 的 JSON 配置包含：`v`(版本), `ps`(名称), `add`(地址), `port`(端口), `id`(UUID), `aid`(alterId), `scy`(加密), `net`(网络), `type`(伪装), `host`, `path`, `tls`, `sni` 等字段。

### 订阅文件格式

部署完成后，Proxy Installer 会在 VPS 上生成 nginx 订阅分发服务，支持 4 种客户端订阅格式：

| 客户端 | 订阅路径 | 文件格式 | 内容说明 |
| --- | --- | --- | --- |
| Shadowrocket | `/sub/{token}/shadowrocket` | 文本 | Base64 编码的标准 URI 列表（\n 分隔） |
| V2rayNG | `/sub/{token}/v2rayng` | 文本 | Base64 编码的标准 URI 列表（\n 分隔） |
| Clash Meta / Mihomo | `/sub/{token}/mihomo.yaml` | YAML | Clash Meta 代理配置 + 代理组规则 |
| sing-box | `/sub/{token}/sing-box.json` | JSON | sing-box 客户端配置（mixed inbound + outbounds） |

> 所有订阅均通过 HTTP 明文传输，建议在可信网络环境下使用。Token 可在高级设置中自定义，默认为 `starter2026`。
...
前往 [Releases](https://github.com/youzhiqingw/proxy-installer/releases/latest) 下载最新版安装包：
...
```bash
git clone https://github.com/youzhiqingw/proxy-installer.git
cd proxy-installer
...
本项目基于 [BUSL 1.1](LICENSE) 开源。非商业/internal 使用免费；商业使用请联系获取授权。
